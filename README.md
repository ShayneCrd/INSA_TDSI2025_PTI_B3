...
<p align="center">
  ...
  <br>
  <em>Figure 1 – Predicted masks from nnU-Net, SegResNet and lst-AI with the ground truth (GT) mask</em>
</p>
...
- **LST-AI**  
  LST-AI is a state-of-the-art automated tool for multiple sclerosis (MS) lesion segmentation.
  It relies on an ensemble of **three pre-trained U-Net architectures**, each trained on large
  and heterogeneous MS datasets.
  In this project, LST-AI is used **exclusively in inference mode**, without any fine-tuning or
  retraining on the target datasets.
  
  As a result, LST-AI serves as a **strong off-the-shelf baseline**, allowing us to evaluate the
  performance of fully pre-trained models against nnU-Net and SegResNet, which are trained
  specifically on the datasets considered in this study.

- **SegResNet (MONAI)**  
  SegResNet is a 3D residual convolutional neural network implemented in the
  **MONAI (Medical Open Network for AI)** framework.
  It combines a U-Net–like encoder–decoder structure with **residual blocks**,
  enabling more stable training and improved gradient flow in deep architectures.
  
  In this project, SegResNet is trained **from scratch** using cross-validation,
  following the data splits generated by nnU-Net.
  This allows a fair comparison between a manually configured deep learning model
  and nnU-Net’s self-configuring pipeline, while leveraging MONAI’s flexible and
  research-oriented design.


- **nnU-Net v2**  
  nnU-Net v2 is a self-configuring segmentation framework based on the U-Net
  architecture, designed to automatically adapt its preprocessing, network
  configuration, and training strategy to a given dataset.
  
  In this project, nnU-Net v2 is used both as a **standalone segmentation method**
  and as a **reference framework for data preprocessing and cross-validation
  splits**.
  The folds generated by nnU-Net are reused for SegResNet training, ensuring
  consistent data splits and enabling a fair comparison between the different
...
Move your un-formatted datasets into the nnUnetFrame directory. In our case: MSLesSeg, MICCAI2016_test, MICCAI2016_train, Open_ms_data-master. It is important not to rename them
...
If these steps ran without any error, we can generate the folds required for for nnU-Net to work (and SegResNet as well).

**For Dataset421:**  

```bash
nnUNetv2_plan_and_preprocess -d 421 
```

**For Dataset422:**  

```bash
nnUNetv2_plan_and_preprocess -d 422
```

**For Dataset423:**  

```bash
nnUNetv2_plan_and_preprocess -d 423 
```

**For Dataset424:**  

```bash
nnUNetv2_plan_and_preprocess -d 424 
```
